        ...
        <div className="mx-auto max-w-7xl px-6">
          <AnimateOnScroll className="text-center mb-16">
            ...
            <h2 className="font-display text-4xl lg:text-5xl tracking-tight">
              Dream &middot; Design &middot; Build &middot; <span className="text-brand italic">Enjoy</span>
            </h2>
          </AnimateOnScroll>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {values.map(v => (
              <StaggerItem key={v.title}>
                <div className="bg-card rounded-2xl border border-border p-8 lg:p-10 h-full text-center">
                  <div className="w-14 h-14 rounded-full border-2 border-brand/20 flex items-center justify-center mb-6 mx-auto">
                    <v.icon size={22} className="text-brand" strokeWidth={1.5} />
                  </div>
                  <h4 className="font-display text-xl text-foreground mb-3">{v.title}</h4>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={stat.label} className={`rounded-2xl p-8 text-center ${i === 0 ? 'bg-foreground text-white' : 'bg-secondary'}`}>
                ...
                <div className={`text-xs font-semibold uppercase tracking-widest mt-3 ${i === 0 ? 'text-white/50' : 'text-muted-foreground'}`}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 lg:py-32">
        <div className="mx-auto max-w-7xl px-6">
          <AnimateOnScroll className="mb-16">
            ...
            <h2 className="font-display text-4xl lg:text-5xl tracking-tight">
              The people behind the <span className="text-brand italic">work</span>
            </h2>
          </AnimateOnScroll>
